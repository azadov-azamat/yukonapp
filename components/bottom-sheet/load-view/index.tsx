...
import { View, Text, Switch, StyleSheet, TouchableOpacity, ActivityIndicator, ScrollView, FlatList } from 'react-native';
    ...
    >
      <BottomSheetView className="relative flex-1">
        ...
        {!loadingLoadById && load && <View className="h-full px-4 ">
          ...
          {/* Load Details */}
          <LoadDetails load={load} t={t} />
          
          {/* Phone Call Button */}
          <LoadFooter load={load} t={t} />
...
const LoadStatus: React.FC<LoadStatusProps> = ({ load, t }) => {
...
const LoadHeader: React.FC<{load: LoadModel, t: TFunction, bottomSheetRef: React.RefObject<BottomSheet>}> = ({ load, t, bottomSheetRef }) => {
  const dispatch = useAppDispatch();
  ...
  return (  
   <>
       <View className="relative flex-row items-center justify-center pt-2 pb-3 border-b border-border-color">
            {/* <Text className="text-lg font-bold">{t('load')}</Text> */}
            <TouchableOpacity className="absolute left-0" onPress={toggle}>
...
const LoadDetails: React.FC<{load: LoadModel, t: TFunction}> = ({ load, t }) => {  
  const { theme } = useTheme();
...
const LoadFooter: React.FC<{load: LoadModel, t: TFunction}> = ({ load, t }) => {  
  const dispatch = useAppDispatch();
  ...
  return (
    ...
    {!load.phone && user && <TouchableOpacity
                              ...
                              onPress={() => load.phoneFunction(user, dispatch).then(toggleDropdown)} 
